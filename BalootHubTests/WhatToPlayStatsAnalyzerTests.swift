...
final class WhatToPlayStatsAnalyzerTests: XCTestCase {
    ...
    func testTrainingSessionProgressStartsNotStartedWithoutMatchingAttempts() {
        ...
        XCTAssertEqual(progress.nextSeed, WhatToPlayStatsAnalyzer.nextTrainingSessionSeed(for: attempts, plan: plan))
        XCTAssertEqual(progress.nextSeedState, .fresh(WhatToPlayStatsAnalyzer.nextTrainingSessionSeed(for: attempts, plan: plan)))
        XCTAssertTrue(progress.nextSeedGuidance?.contains("جديد داخل خطة الجلسة الحالية.".localized) == true)
    ...
    func testTrainingSessionReviewExplainsRetryingIncorrectNextSeed() {
        ...
        XCTAssertEqual(progress.nextSeed, 901)
        XCTAssertEqual(progress.nextSeedState, .retry(901))
        XCTAssertTrue(progress.nextSeedGuidance?.contains("\("أعد الموقف".localized) 901") == true)
        XCTAssertTrue(progress.nextSeedGuidance?.contains("آخر محاولة عليه كانت غير صحيحة".localized) == true)
