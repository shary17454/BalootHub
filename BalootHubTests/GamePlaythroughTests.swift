...
final class GamePlaythroughTests: XCTestCase {
    ...
    /// - **حكم**: 152 نقطة أوراق، ويُضاف 20 لكل مشروع "بلوت" (ملك وبنت الحكم في يد واحدة)،
    ///   فيكون المجموع 152 أو 172 حسب التوزيعة.
    /// - **صن**: 130 نقطة أساسية تُضاعف ×2 حسب العُرف المعتمد في التطبيق ⇒ 260.
    func testRoundTotalsMatchBalootRules() throws {
        for suit in Suit.allCases {
            ...
            // 152 نقطة أوراق + (0 أو 20 أو 40) من مشاريع البلوت المكتشفة.
            XCTAssertTrue([152, 172, 192].contains(total),
                          "حكم \(suit): المجموع \(total) لا يطابق 152 + مضاعفات مشروع البلوت")
            XCTAssertEqual((total - 152) % 20, 0, "حكم \(suit): الفائض ليس من مضاعفات 20")
        }
