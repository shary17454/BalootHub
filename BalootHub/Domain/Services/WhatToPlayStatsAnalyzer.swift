...
enum WhatToPlayStatsAnalyzer {
    ...
    static func reviewQueue(for attempts: [WhatToPlayAttempt], limit: Int = 3) -> [WhatToPlayReviewItem] {
        ...
        .map { attempt in
            ...
            return WhatToPlayReviewItem(
                ...
                secondBestSimulationCard: attempt.secondBestSimulationCard,
    ...
    static func trainingSessionReview(
        ...
        case .continueSession:
            ...
            return WhatToPlayTrainingSessionReview(
                ...
                detail: trainingSessionNextSeedDetail(
                    progress.nextStepDetail,
                    nextSeed: nextSeed,
                    attempts: attempts,
                    plan: plan
                ),
    ...
    private static func trainingSessionNextSeedDetail(
        ...
        guard let latest = latestMatchingTrainingAttempt(seed: nextSeed, attempts: attempts, plan: plan),
              !latest.isCorrect
    ...
    private static func latestMatchingTrainingAttempt(
        ...
        uniqueMatchingTrainingAttempts(for: attempts, plan: plan)
            .first { $0.replaySeed == seed }
    }
    ...
    static func valueLossSeverity(
        ...
        valueLossSeverity(
            for: WhatToPlayValueLossSeverityCategory.classify(
                decisiveLoss: max(
                    lostExpectedPoints,
                    lostProjectedTeamPoints,
                    lostProjectedAgainstSecondBestPoints
                )
            )
        )
    }
    ...
    static func retryPrompt(for selected: WhatToPlayOption, in scenario: WhatToPlayScenario) -> WhatToPlayRetryPrompt? {
        ...
        return retryPrompt(
            ...
            detailSuffix: retryPromptSimulationDetail(
                ...
                lostExpectedPoints: recommendation.lostExpectedPoints
            )
        )
    }
    ...
    private static func retryPrompt(
        ...
        case .smallGapPractice:
            return WhatToPlayRetryPrompt(
                ...
                detail: retryPromptDetail(
                    base: "الفرق بسيط؛ أعد الموقف مرة واحدة وحاول تمييز سبب ترجيح الخبير للورقة الأفضل.".localized,
                    suffix: detailSuffix
                ),
                iconName: "arrow.counterclockwise.circle.fill",
        ...
        case .replayUncounted:
            return WhatToPlayRetryPrompt(
                title: "أعد نفس الموقف".localized,
                detail: retryPromptDetail(
                    base: "لن تُحسب الإعادة كمحاولة جديدة. ركّز على الورقة الأفضل قبل الانتقال للموقف التالي.".localized,
                    suffix: detailSuffix
                ),
                iconName: "arrow.counterclockwise.circle.fill",
                recommendedCard: bestCard,
    ...
    private static func retryPromptDetail(base: String, suffix: String?) -> String {
        guard let suffix, !suffix.isEmpty else { return base }
        return "\(base) \(suffix)"
    }

    private static func retryPromptSimulationDetail(
        lostProjectedTeamPoints: Int,
        ...
        guard simulationLoss > lostExpectedPoints, simulationLoss > 0 else { return nil }
        return simulationLossText(
            projectedLost: lostProjectedTeamPoints,
            secondProjectedLost: lostProjectedAgainstSecondBestPoints
        )
    }
