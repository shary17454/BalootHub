    ...
    static func refresh(context: ModelContext, saveImmediately: Bool = true) throws {
        ...
        for definition in Self.allDefinitions {
            if let existing = existingBySlug[definition.slug] {
                didChange = definition.apply(to: existing) || didChange
            } else {
...
private struct GameDefinition {
    ...
    @discardableResult
    func apply(to item: GameCatalogItem) -> Bool {
        var didChange = false

        func assign<T: Equatable>(_ keyPath: ReferenceWritableKeyPath<GameCatalogItem, T>, _ value: T) {
        ...
        assign(\.arabicTitle, arabicTitle)
        assign(\.englishTitle, englishTitle)
        assign(\.shortDescription, shortDescription)
        assign(\.category, category)
        assign(\.playerCountText, playerCountText)
        assign(\.difficulty, difficulty)
        assign(\.estimatedDuration, estimatedDuration)
        assign(\.iconName, iconName)
        assign(\.accentToken, accentToken)
        assign(\.isPlayable, isPlayable)
        ...
        for kind in StandardRuleSectionKind.allCases {
            ...
            if let section = item.ruleSection(kind) {
                ...
                if section.order != kind.order {
                    section.order = kind.order
                    didChange = true
                }
                if section.iconName != kind.iconName {
...
private extension CatalogSeeder {
    static let allDefinitions: [GameDefinition] = [
        ...
        GameDefinition(
            ...
            shortDescription: "كوت بستة لاعبين وفريقين: مزايدة من 5 إلى 9 مع جوكرين وحكم يختاره صاحب العقد.",
            category: .otherCardGame,
            playerCountText: "6 لاعبين (فريقان)",
            difficulty: .intermediate,
            ...
            sortOrder: 50,
            sections: [
                .objective: "تحقيق العقد قبل الخصم. الهدف 101، أو 51 مقابل صفر؛ نجاح عقد 9 في أول جولة يحسم المباراة.",
                .playerCount: "6 لاعبين (فريقان)",
                .setup: "54 ورقة؛ 9 لكل لاعب، والشركاء في المقاعد المتناوبة.",
                .dealing: "54 ورقة؛ 9 لكل لاعب، والشركاء في المقاعد المتناوبة.",
                .cardRanking: "الجوكر الأحمر ثم آس الحكم ثم الجوكر الأسود ثم بقية الحكم؛ بعدها النوع المطلوب من الآس إلى 2.",
                .howToPlay: "مزايدة واحدة من 5 إلى 9؛ آخر الممررين ملزوم 5. اتبع النوع إن وجد، والجوكر مسموح أثناء الأكلة.",
                .scoring: "نجاح العقد يمنح قيمته، وعقد 9 يمنح 36. الفشل يمنح الخصم ضعف العقد، أو 5 في الملزوم.",
                .projects: "لا توجد مشاريع بالمفهوم المستخدم في البلوت.",
                .roundEnd: "تحقيق العقد قبل الخصم. الهدف 101، أو 51 مقابل صفر؛ نجاح عقد 9 في أول جولة يحسم المباراة.",
                .commonMistakes: "بدء الجوكر مقيد بخروج كبار الحكم؛ عقد 8 يسمح بالأحمر وعقد 9 بكليهما. الجوكر المجبور على الافتتاح يفقد قوته."
            ]
        ),
        GameDefinition(
            slug: "tarneeb",
        ...
        GameDefinition(
            ...
            sortOrder: 52,
            sections: [
                .objective: "تركس أفراد: أربع ممالك، لكل لاعب خمسة عقود مختلفة؛ يفوز أعلى مجموع بعد 20 توزيعة، دون تدبيل.",
                ...
                .cardRanking: "اتبع النوع؛ الآس الأعلى بلا حكم. في عقد شايب الهاص لا تفتح بالهاص إلا إذا كانت يدك كلها هاص.",
                .howToPlay: "مالك 7 الهاص يبدأ المملكة. في عقد تركس ابدأ بالشباب ثم وسع التسلسل إلى 2 والآس؛ مرر فقط عند انعدام الحركة.",
                .scoring: "شايب الهاص -75؛ كل ديمن -10؛ كل بنت -25؛ كل أكلة -15. ترتيب إنهاء تركس: 200، 150، 100، 50.",
                .projects: "لا توجد مشاريع بالمفهوم المستخدم في البلوت.",
                .roundEnd: "تركس أفراد: أربع ممالك، لكل لاعب خمسة عقود مختلفة؛ يفوز أعلى مجموع بعد 20 توزيعة، دون تدبيل.",
        ...
        GameDefinition(
            ...
            arabicTitle: "هاند",
            englishTitle: "Hand",
            shortDescription: "هاند بأربع أياد وحزمتين وجوكرين: سحب، إنزال مجموعات وتسلسلات، تركيب ثم رمي.",
            category: .otherCardGame,
            playerCountText: "4 لاعبين أفراد",
            difficulty: .beginner,
            ...
            sections: [
                ...
                .playerCount: "4 لاعبين أفراد",
                .setup: "106 أوراق؛ 14 لكل لاعب و15 للبادي الذي يرمي أولا. خمس جولات، والأقل نقاطا يفوز.",
                .dealing: "106 أوراق؛ 14 لكل لاعب و15 للبادي الذي يرمي أولا. خمس جولات، والأقل نقاطا يفوز.",
                .cardRanking: "المجموعة رتبة واحدة بأنواع مختلفة، والتسلسل نوع واحد متتابع؛ ثلاث أوراق فأكثر. الآس منخفض أو مرتفع بلا التفاف.",
                .howToPlay: "الإنزال الأول 51 على الأقل. اسحب المكشوف فقط لاستخدامه في مجموعة جديدة؛ بعد الإنزال يمكنك التركيب، واحتفظ بورقة للرمي.",
                .scoring: "الفائز -30؛ الباقون قيمة أوراقهم و100 إضافية لمن لم ينزل. الآس 11 والجوكر 15 والصور 10؛ الهاند الكامل يضاعف النتائج.",
                .projects: "المجموعة رتبة واحدة بأنواع مختلفة، والتسلسل نوع واحد متتابع؛ ثلاث أوراق فأكثر. الآس منخفض أو مرتفع بلا التفاف.",
                .roundEnd: "106 أوراق؛ 14 لكل لاعب و15 للبادي الذي يرمي أولا. خمس جولات، والأقل نقاطا يفوز.",
                .commonMistakes: "الإنزال الأول 51 على الأقل. اسحب المكشوف فقط لاستخدامه في مجموعة جديدة؛ بعد الإنزال يمكنك التركيب، واحتفظ بورقة للرمي."
            ]
        )
