...
struct HandAnalysisTests {
    ...
    func strongHokumHandRecommendsTrumpSuit() {
        ...
        #expect(analysis.bidOptions.first?.rationale.contains("سبيت") == true)
        #expect(analysis.nextActionTitle.contains("شراء"))
        #expect(analysis.nextActionDetail.contains("حكم سبيت"))
        #expect(analysis.sunHokumScoreGap < 0)
        #expect(analysis.modeComparisonTitle.contains("الحكم"))
        #expect(analysis.modeComparisonDetail.contains("سبيت"))
        #expect(analysis.strengths.contains { $0.contains("سبيت") })
        #expect(analysis.strengths.contains { $0.contains("الولد") })
        #expect(analysis.tacticalAdvice.contains("حكم سبيت"))
    }
